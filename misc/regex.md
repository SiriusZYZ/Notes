...
**子模式：** `(pattern)`相当于把某个模式看作一个整体，这样就能对其进行一些”乘法操作“，这和数学运算中的括号比较类似。
...
| 限定符            | 等价             | 描述                      |
| -------------- | -------------- | ----------------------- |
| `pattern{n}`   |                | `pattern` 严格重复n次        |
| `pattern{n,}`  |                | `pattern` 至少重复n次        |
| `pattern{n,m}` |                | `pattern` 至少重复n次，至多重复m次 |
| `pattern*`     | `pattern{0,}`  | `pattern` 重复零次或多次       |
| `pattern+`     | `pattern{1,}`  | `pattern` 至少重复1次        |
| `pattern?`     | `pattern{0,1}` | `pattern` 重复0次或1次       |
\* 备注: `n` 非负且`n<=m`

**贪婪匹配：** 指尽可能增加匹配次数，例如`o+` 对`google`的匹配，你既可以每次匹配一个`o`，也可以尽可能重复模式，即匹配`oo`。不同语言对限定符的”贪婪“好像是不一样的。

```python
# python默认尽可能多重复模式
>>> re.findall("o+", "gooooof.")
['ooooo']
# 在限定符后再加上一个+, 表示尽可能重复模式
>>> re.findall("o++", "gooooof.")
['ooooo']
# 在限定符后再加上一个?, 表示尽可能不重复模式
>>> re.findall("o+?", "gooooof.")
['o', 'o', 'o', 'o', 'o']
```

### 模式或运算
**或运算**：有时会需要匹配两个模式中的任意一个，这个时候就需要或运算了，语法是`patternA|patternB`

```python
>>> re.findall("cat|dog", "happy cat and happy dog")
['cat', 'dog']
```
...
| `i (ignore)`    | 不区分大小写                                        |
| `a (ASCII)`     | 仅匹配ASCII                                      |
| `m (multiline)` | 允许多行匹配，变更`^` 和`$` 的行为，见[匹配位置](#匹配位置)          |
| `s (dotall)`    | 使 `.` 特殊字符匹配任意字符，包括换行和回车符，见[前文](#匹配一组特定类型的字符) |
